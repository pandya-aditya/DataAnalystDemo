function isNexusAgent(agent) {
  const a = String(agent || '').trim().toLowerCase()
  return a === 'nexus'
}

function shouldBlueHighlightAction(action) {
  if (action?.accent === 'blue') return true
  const agent = action?.agent
  if (!agent || !String(agent).trim()) return false
  return !isNexusAgent(agent)
}
    ...
    <aside id="context-panel" className={`context-panel${open ? ' open' : ''}`}>
      ...
      <div className="context-body">
        <div className="context-section">
          ...
          {actions.length > 0 ? (
            actions.map(action => {
              const blueHighlight = shouldBlueHighlightAction(action)
              return (
                <div
                  key={action.id}
                  className={`action-item${blueHighlight ? ' accent-blue' : ''}`}
                >
                  <div className="action-main">
                    {action.agent ? <span className="action-agent">{action.agent}</span> : null}
                    <span className="action-name">{action.text}</span>
                    {action.time ? <span className="action-time">{action.time}</span> : null}
                  </div>
                  <div className="action-right">
                    <span
                      className={`action-status${action.status === 'reverted' ? ' reverted' : ''}${blueHighlight ? ' accent-blue' : ''}`}
                    >
